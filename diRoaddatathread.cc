...
RoadDataThread::RoadDataThread(const std::vector<road::diStation> & in_stations_to_plot,
...
#ifdef DEBUGPRINT
  //  std::cerr << "++ Roaddatathread::getNextJob( " << info.stationindex_ << ",
  //  " << info.obstime_ << " ) ++" << std::endl;
#endif
  jobs.pop();
  noOfJobs--;
  return info;
...
void RoadDataThread::addJob(int stationindex, const miTime & obstime)
{
  //  the vector works like a FIFO
#ifdef DEBUGPRINT
  //  std::cerr << "++ Roaddatathread::addJob( " << stationindex << ", " <<
  //  obstime << " ) ++" << std::endl;
#endif
  jobInfo info;
...
{
#ifdef DEBUGPRINT
  std::cerr << "++ Roaddatathread::run() ++" << std::endl;
#endif
...
retry:
  try {
    ...
    std::cerr << "Roaddatathread::run(), Connection to db failed!, " << connect_str.c_str()
              << std::endl;
#ifdef DEBUGPRINT
    std::cerr << "++ Roaddatathread::run() done ++" << std::endl;
#endif
  ...
  while (!stop) {
    if (getJobSize() > 0) {
      ...
      std::map<int, std::string>::iterator itm =
        diStation::dataproviders[stationfile_].begin();
      ...
      if ((*stations)[i].station_type() == road::diStation::WMO) {
        sprintf(buf, "%s|%d|%s|%s|%f|%f", (*stations)[i].name().c_str(),
        ...
        sprintf(buf, "%s|%s|%s|%s|%f|%f", (*stations)[i].name().c_str(),
          (*stations)[i].ICAOID().c_str(), (char *) obstime_.isoDate().c_str(),
          (char *) obstime_.isoClock(true, true).c_str(), (*stations)[i].lat(),
          (*stations)[i].lon());
      }
      else if ((*stations)[i].station_type() == road::diStation::SHIP) {
        //  Call signs have trailing space and space between name parts. They
        //  must bee removed and replaced.
        std::string call_sign_ = (*stations)[i].call_sign();
        //  call_sign_.trim(true, true, " ");
        //  call_sign_.replace(" ", "_");
        sprintf(buf, "%s|%s|%s|%s", call_sign_.c_str(), call_sign_.c_str(),
          (char *) obstime_.isoDate().c_str(),
          (char *) obstime_.isoClock(true, true).c_str());
      }
      std::string line(buf);
      ...
      if (itd != tmp_data.end()) {
        ...
        if (toplot) {
          ...
          if (itm != diStation::dataproviders[stationfile_].end()) {
            ...
            try {
              if ((*stations)[i].station_type() == road::diStation::WMO) {
                ...
                //  (Queries)
                std::string icaoid = (*stations)[i].ICAOID();
                //  Search for surface stations (wmo_station_type_code = 0)
                sprintf(query, "SELECT * FROM \
									...
									WHERE validtime_to>'%s' AND icao_code = '%s';",
                  (char *) obstime_.isoTime(true, true).c_str(), icaoid.c_str());
              }
              else if ((*stations)[i].station_type() == road::diStation::SHIP) {
                //  (Queries)
                std::string callsign = (*stations)[i].call_sign();
                //  Search for surface stations (wmo_station_type_code = 0)
                sprintf(query,
                  "SELECT trim(ship.ship_id) AS ship_id, ship.sender_id AS sender_id \
									FROM ship_view ship \
									WHERE trim(ship.ship_id) = '%s';",
                  callsign.c_str());
              }
              ...
              if (!res.empty()) {
#ifdef DEBUGPRINT
                std::cerr << "Size of result set: " << res.size() << std::endl;
#endif
...
#endif
                  if ((*stations)[i].station_type() == road::diStation::WMO
                    || (*stations)[i].station_type() == road::diStation::ICAO) {
                    if (m == 0) {
              ...
              std::cerr << "Roaddatathread::run(), Getting placeid failed!" << std::endl;
              std::cerr << "query: " << query << std::endl;
#ifdef DEBUGPRINT
              std::cerr << "++ Roaddatathread::run() done ++" << std::endl;
#endif
          ...
          try {
            ...
            nTmpRows = 0;
            //  select * from kvalobs_wmo_observation_view where position_id=747
            //  and parameter_id in(4,3);
            if ((*stations)[i].station_type() == road::diStation::WMO) {
...
FROM wmo_station_identity_view wmo join stationary_observation_value_context_view ctx on wmo.position_id = ctx.position_id join stationary_observation_place_view pos on wmo.position_id = pos.position_id  join stationary_observation_value_approved_view val on val.observation_master_id = ctx.observation_master_id and val.time_tick between wmo.validtime_from and wmo.validtime_to and val.real_time_store=true join statistics_formula_view f on f.statistics_formula_id = ctx.statistics_formula_id join parameter_view par on par.parameter_id = ctx.parameter_id join level_combination_view lc on lc.level_combination_id = ctx.level_combination_id join level_parameter_view lp on lp.level_parameter_id = lc.level_parameter_id) AS diana_wmo_observation_wiew WHERE observation_master_id in (%s) and valid_to='%s' and reference_time='%s';",
                  ...
                  (char *) refTime.isoTime(true, true).c_str());
              }
              else {
                if (minute == 0) {
                  sprintf(query,
                    "SELECT * FROM (SELECT wmo.wmo_block_number AS "
                    "wmo_block, wmo.wmo_station_number AS wmo_number, "
                    "wmo.position_id, "
                    "round(st_y(pos.stationary_position)::numeric, 2) AS "
                    "lat, round(st_x(pos.stationary_position)::numeric, 2) "
                    "AS lon, ctx.parameter_id, lc.level_parameter_id, "
                    "lc.level_from, lc.level_to, lp.unit_name AS "
                    "level_parameter_unit_name, f.statistics_formula_name AS "
                    "statistics_type, val.observation_value AS value, "
                    "par.parameter_unit AS parameter_unit, val.quality, "
                    "val.station_operating_mode AS automation_code, "
                    "val.time_tick AS reference_time, val.time_tick - "
                    "val.offset_from_time_tick - "
                    "ctx.observation_sampling_time AS valid_from, "
                    "min(val.offset_from_time_tick) AS valid_offset, "
                    "val.time_tick - val.offset_from_time_tick AS valid_to, "
                    "ctx.observation_master_id, val.time_tick, "
                    "date_part('epoch'::text, ctx.observation_sampling_time) "
                    "AS observation_sampling_time_seconds, "
                    "date_part('epoch'::text, val.offset_from_time_tick) AS "
                    "offset_from_time_tick_seconds, val.value_version_number "
                    "FROM wmo_station_identity_view wmo join "
                    "stationary_observation_value_context_view ctx on "
                    "wmo.position_id = ctx.position_id join "
                    "stationary_observation_place_view pos on "
                    "wmo.position_id = pos.position_id join "
                    "stationary_observation_value_approved_view val on "
                    "val.observation_master_id = ctx.observation_master_id "
                    "and val.time_tick between wmo.validtime_from and "
                    "wmo.validtime_to and val.real_time_store=true join "
                    "statistics_formula_view f on f.statistics_formula_id = "
                    "ctx.statistics_formula_id join parameter_view par on "
                    "par.parameter_id = ctx.parameter_id join "
                    "level_combination_view lc on lc.level_combination_id = "
                    "ctx.level_combination_id join level_parameter_view lp "
                    "on lp.level_parameter_id = lc.level_parameter_id GROUP "
                    "BY wmo.wmo_block_number, wmo.wmo_station_number, "
                    "wmo.position_id, "
                    "pos.stationary_position,ctx.parameter_id, "
                    "lc.level_parameter_id, lc.level_from, lc.level_to, "
                    "lp.unit_name, f.statistics_formula_name, "
                    "val.observation_value,par.parameter_unit, val.quality, "
                    "val.station_operating_mode,  val.time_tick, "
                    "val.offset_from_time_tick, "
                    "ctx.observation_sampling_time, "
                    "ctx.observation_master_id, val.value_version_number) AS "
                    "diana_wmo_observation_wiew WHERE position_id in(%s) and "
                    "parameter_id in(%s) and valid_to + valid_offset ='%s'  "
                    "and  reference_time='%s';",
                    (char *)
                      diStation::dataproviders[stationfile_][(*stations)[i].stationID()]
                        .c_str(),
                  ...
                  sprintf(query,
                    ...
                    "diana_wmo_observation_wiew WHERE position_id in(%s) and "
                    "parameter_id in(%s) and valid_to + valid_offset ='%s'  "
                    "and  reference_time='%s' and valid_offset = '%d "
                    "minutes'::interval;",
                    (char *)
                      diStation::dataproviders[stationfile_][(*stations)[i].stationID()]
                        .c_str(),
                    parameters, (char *) refTime.isoTime(true, true).c_str(),
                    (char *) refTime.isoTime(true, true).c_str(), minute);
                }
                /*
                sprintf(query,
                        "SELECT * FROM(SELECT wmo.wmo_block_number AS wmo_block,
wmo.wmo_station_number AS wmo_number, wmo.position_id,
round(st_y(pos.stationary_position)::numeric, 2) AS lat,
round(st_x(pos.stationary_position)::numeric, 2) AS lon,ctx.parameter_id,
lc.level_parameter_id, lc.level_from, lc.level_to, lp.unit_name AS
level_parameter_unit_name, f.statistics_formula_name AS statistics_type,
val.observation_value AS value, par.parameter_unit AS parameter_unit,
val.quality, val.station_operating_mode AS automation_code, val.time_tick AS
reference_time, val.time_tick - val.offset_from_time_tick -
ctx.observation_sampling_time AS valid_from, val.time_tick -
val.offset_from_time_tick AS valid_to, ctx.observation_master_id, val.time_tick,
date_part('epoch'::text, ctx.observation_sampling_time) AS
observation_sampling_time_seconds, date_part('epoch'::text,
val.offset_from_time_tick) AS offset_from_time_tick_seconds,
val.value_version_number \ FROM wmo_station_identity_view wmo join
stationary_observation_value_context_view ctx on wmo.position_id =
ctx.position_id join stationary_observation_place_view pos on wmo.position_id =
pos.position_id  join stationary_observation_value_approved_view val on
val.observation_master_id = ctx.observation_master_id and val.time_tick between
wmo.validtime_from and wmo.validtime_to and val.real_time_store=true join
statistics_formula_view f on f.statistics_formula_id = ctx.statistics_formula_id
join parameter_view par on par.parameter_id = ctx.parameter_id join
level_combination_view lc on lc.level_combination_id = ctx.level_combination_id
join level_parameter_view lp on lp.level_parameter_id = lc.level_parameter_id)
AS diana_wmo_observation_wiew WHERE position_id in(%s) and parameter_id in(%s)
and valid_to='%s' and reference_time='%s';"
,(char
*)diStation::dataproviders[stationfile_][(*stations)[i].stationID()].c_str(),
parameters, (char *)obstime_.isoTime(true,true).c_str(),(char
*)refTime.isoTime(true,true).c_str());
*/
              }
            }
            else if ((*stations)[i].station_type() == road::diStation::ICAO) {
              //  A metar station report every 30 minutes, x.20, x.50, so we must
              //  use valid_to to get the data. We construct a time tick for
              //  performance reasons.
              miTime refTime = obstime_;
              miClock refClock = refTime.clock();
              miDate refDate = refTime.date();
              ...
              refTime.setTime(refDate, refClock);
              refTime.addHour(+1);
              //  std::cerr << "obstime,reftime: " << (char
              //  *)obstime_.isoTime(true,true).c_str()<< ", " << (char
              //  *)refTime.isoTime(true,true).c_str()<< std::endl;
              if (strlen(observation_master_ids)) {
                sprintf(query,
...
AS diana_ship_observation_wiew where sender_id in (%s) and parameter_id in(%s) and reference_time='%s') as agg WHERE min_offset_from_time_tick_seconds = offset_from_time_tick_seconds;",
            ...
            if (!res.empty()) {
              nTmpRows = res.size();
#ifdef DEBUGPRINT
              std::cerr << "Size of result set: " << nTmpRows << std::endl;
#endif
              //  write operations
              (*stations)[i].setData(true);
              for (int m = 0; m < nTmpRows; m++) {
                ...
                if ((*stations)[i].station_type() == road::diStation::WMO) {
                  std::cout << row[wmo_block].c_str() << ": " << row[wmo_number].c_str()
                            ...
                            << ": " << row[level_parameter_id].c_str() << ": "
                            << row[level_from].c_str() << ": " << row[level_to].c_str()
                            << ": " << row[level_parameter_unit_name].c_str() << ": "
                            << row[statistics_type].c_str() << ": " << row[value].c_str()
                            << ": " << row[unit].c_str() << ": " << row[quality].c_str()
                            << ": " << row[automation_code].c_str() << ": "
                            << row[reference_time].c_str() << ": " << row[valid_from].c_str()
                            << ": " << row[valid_to].c_str() << ": "
                  ...
                  std::cout << row[0].c_str() << ": " << row[position_id - 1].c_str() << ": "
                            << row[lat - 1].c_str() << ": " << row[lon - 1].c_str() << ": "
                            << row[parameter_id - 1].c_str() << ": "
                            << row[level_parameter_id - 1].c_str() << ": "
                            << row[level_from - 1].c_str() << ": "
                            << row[level_to - 1].c_str() << ": "
                            << row[level_parameter_unit_name - 1].c_str() << ": "
                            << row[statistics_type - 1].c_str() << ": "
                            << row[value - 1].c_str() << ": " << row[unit - 1].c_str() << ": "
                            << row[quality - 1].c_str() << ": "
                            << row[automation_code - 1].c_str() << ": "
                            << row[reference_time - 1].c_str() << ": "
                            << row[valid_from - 1].c_str() << ": "
                            << row[valid_to - 1].c_str() << ": "
                            << row[observation_master_id - 1].c_str() << ": "
                            << row[time_tick - 1].c_str() << ": "
                            << row[observation_sampling_time - 1].c_str() << ": "
                            << row[offset_from_time_tick - 1].c_str() << ": "
                            << row[value_version_number - 1].c_str() << std::endl;
                }
                else if ((*stations)[i].station_type() == road::diStation::SHIP) {
                  ...
                  //  ZQSD5,3281,58.30,0.80,1006,104,0,0,metre,Instantaneous,102060,pascal,Nordklim:89999,0,2013-10-14
                  //  01:00:00,2013-10-14 01:00:00,2013-10-14
                  //  01:00:00,140824,2013-10-14 01:00:00,0,0,0

                  std::cout << row[s_ship_id].c_str() << ": " << row[s_sender_id].c_str()
                            << ": " << row[s_lat].c_str() << ": " << row[s_lon].c_str()
                            << ": " << row[s_parameter_id].c_str() << ": "
                            << row[s_level_parameter_id].c_str() << ": "
                            << row[s_level_from].c_str() << ": " << row[s_level_to].c_str()
                            << ": " << row[s_level_parameter_unit_name].c_str() << ": "
                            << row[s_statistics_type].c_str() << ": " << row[s_value].c_str()
                            << ": " << row[s_parameter_unit].c_str() << ": "
                            << row[s_quality].c_str() << ": "
                            << row[s_automation_code].c_str() << ": "
                            << row[s_reference_time].c_str() << ": "
                            << row[s_valid_from].c_str() << ": " << row[s_valid_to].c_str()
                            << ": " << row[s_observation_master_id].c_str() << ": "
                            << row[s_time_tick].c_str() << ": "
                            << row[s_observation_sampling_time_seconds].c_str() << ": "
                            << row[s_offset_from_time_tick_seconds].c_str() << ": "
                            << row[s_value_version_number].c_str() << std::endl;
                }
#endif
                ...
                //  values are double.
                //  0,1,273.05,0,0,1312,1312,,0,60.64,12.88,2,2,2010-05-27
                //  10:30:00,2010-05-27 10:30:00,2010-05-27
                //  11:00:00,1006,9,5,0,0,kelvin,2010-05-27 11:00:00,2,0,RDKINST
                if ((*stations)[i].station_type() == road::diStation::WMO) {
                  crow.integervalue = 0;
                ...
                std::cerr << crow.integervalue << "," << crow.integervalue_is_null << ","
                          ...
                          << crow.origingeoindex << "," << crow.geop.lat << ","
                          << crow.geop.lon << "," << crow.altitudefrom << ","
                          << crow.altitudeto << "," << crow.validtimefrom << ","
                          << crow.validtimeto << "," << crow.reftime << "," << crow.srid
                          << "," << crow.quality << "," << crow.parameter << ","
                          << crow.parametercodespace << "," << crow.levelparametercodespace
                          << "," << crow.unit << "," << crow.storetime << ","
                          << crow.dataversion << "," << crow.automationcode << ","
                          << crow.statisticstype << std::endl;
                std::cerr << std::endl;
#endif
                ...
                for (int k = 0; k < params->size(); k++) {
                  if ((*params)[k].isMapped(crow)) {
                    ...
                    tmpresult[k] = std::string(tmpBuf);
#ifdef DEBUGPRINT
                    std::cerr << "Param: " << (*params)[k].diananame()
                              << " value: " << tmpresult[k] << std::endl;
#endif
                    break;
                  }
                }
            ...
            //  If we get here, Xaction has been rolled back
            std::cerr << "Roaddatathread::run(), Getting data failed!" << std::endl;
            std::cerr << "query: " << query << std::endl;
#ifdef DEBUGPRINT
            std::cerr << "++ Roaddatathread::run() done ++" << std::endl;
#endif
          ...
          if ((*stations)[i].station_type() == road::diStation::WMO) {
            fprintf(stderr,
              "station: %i, get data from mora took %6.4f  seconds, %d "
              "rows retrieved\n",
              (*stations)[i].wmonr(),
            ...
            fprintf(stderr,
              "station: %s, get data from mora took %6.4f  seconds, %d "
              "rows retrieved\n",
              (*stations)[i].ICAOID().c_str(),
              ((double) ((tend.time * 1000 + tend.millitm)
                 - (tstart.time * 1000 + tstart.millitm))
                / 1000),
              nTmpRows);
...
#endif
          if ((*stations)[i].station_type() == road::diStation::SHIP) {
            line = line + "|" + from_number((*stations)[i].lat()) + "|"
              + from_number((*stations)[i].lon());
          }
          //  Skip emty data.
          if ((*stations)[i].data()) {
            line = line + "|" + (*stations)[i].station_type() + "|"
              + from_number((*stations)[i].environmentid()) + "|"
              + from_number((*stations)[i].data());
